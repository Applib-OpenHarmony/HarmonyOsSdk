...
function compileClassDeclaration(compiler, stmt) {
    ...
    for (; propertyIndex < properties.length; propertyIndex++) {
        ...
        if (prop.getKind() == properties_1.PropertyKind.Variable) {
            ...
            if (ts.isMethodDeclaration(prop.getValue())) {
                var methodLiteral = new literal_1.Literal(literal_1.LiteralTag.METHOD, compiler.getCompilerDriver().getFuncInternalName(prop.getValue(), compiler.getRecorder()));
                var affiliateLiteral = new literal_1.Literal(literal_1.LiteralTag.METHODAFFILIATE, util_1.getParamLengthOfFunc(prop.getValue()));
    ...
    var nameLiteral = new literal_1.Literal(literal_1.LiteralTag.INTEGER, hasConstructor ? notStaticItemsNum - 1 : notStaticItemsNum);
    ...
    if (stmt.name) {
        ...
        if (!ts.isClassExpression(stmt) && classScope.getParent() instanceof scope_1.GlobalScope) {
...
function compileUnCompiledProperty(compiler, properties, classReg) {
    var pandaGen = compiler.getPandaGen();
...
function createClassLiteralBuf(compiler, classBuffer, stmt, vregs) {
    ...
    var parameterLength = util_1.getParameterLength4Ctor(stmt);
...
function compileConstructor(compiler, node, unreachableFlag) {
    ...
    for (var index = 0; index < members.length; index++) {
        ...
        if (ts.isPropertyDeclaration(decl) && !jshelpers.hasStaticModifier(decl)) {
            var lref = lreference_1.LReference.generateLReference(compiler, decl.name, true);
            if (decl.initializer) {
...
function compileSuperCall(compiler, node, args, hasSpread) {
    ...
    if (scope && level >= 0) {
        ...
        while (tmpScope != scope) {
            if (tmpScope instanceof scope_1.VariableScope) {
                needSetLexVar = true;
            }
        ...
        if (needSetLexVar && curScope instanceof scope_1.FunctionScope) {
...
function loadCtorObj(node, compiler) {
    ...
    if (ts.isConstructorDeclaration(nearestFunc)) {
        ...
        pandaGen.loadAccumulator(node, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.FUNC));
    }
    else {
                ...
                if (ts.isComputedPropertyName(member.name)) {
                    ...
                    if (initializer) {
                        if (properties_1.isConstantExpr(initializer)) {
                            if (defineClassMember(memberName, initializer, properties_1.PropertyKind.Constant, properties, namedPropertyMap)) {
                                staticNum++;
                            }
                        }
                        else {
                            if (defineClassMember(memberName, initializer, properties_1.PropertyKind.Variable, properties, namedPropertyMap)) {
                                staticNum++;
                            }
                        }
                    }
...
function shouldReturnThisForConstruct(stmt) {
    ...
    if (!expr || util_1.isUndefinedIdentifier(expr) || expr.kind == ts.SyntaxKind.ThisKeyword) {
        return true;
    }
    return false;
}
