...
var Recorder = /** @class */ (function () {
        ...
        node.forEachChild(function (childNode) {
            ...
            switch (childNode.kind) {
                ...
                case typescript_1["default"].SyntaxKind.ForOfStatement: {
                    var loopScope = new scope_1.LoopScope(scope);
                    ;
                    _this.setScopeMap(childNode, loopScope);
                    _this.recordInfo(childNode, loopScope);
                    break;
                }
    ...
    Recorder.prototype.recordVariableDecl = function (id, scope) {
        ...
        if (parent) {
            ...
            var declScope = scope.findDeclPos(name);
            if (declScope) {
                var decl = declScope.getDecl(name);
                if ((decl instanceof scope_1.LetDecl || decl instanceof scope_1.ConstDecl)) {
                    var nearestRefVariableScope = scope.getNearestVariableScope();
                    var nearestDefLexicalScope = declScope.getNearestLexicalScope();
                    var tmp = nearestRefVariableScope.getNearestLexicalScope();
                    var needCreateLoopEnv = false;
                    if (nearestDefLexicalScope instanceof scope_1.LoopScope) {
                        while (tmp) {
    ...
    Recorder.prototype.recordImportInfo = function (node, scope) {
        ...
        if (node.importClause) {
            ...
            if (importClause.name) {
                ...
                scope.setDecls(new scope_1.ConstDecl(name_2, importClause.name));
            ...
            if (importClause.namedBindings) {
                ...
                if (typescript_1["default"].isNamedImports(namedBindings)) {
                    namedBindings.elements.forEach(function (element) {
                        ...
                        scope.setDecls(new scope_1.ConstDecl(name, element));
    ...
    Recorder.prototype.recordFuncName = function (node) {
        ...
        if (name != '') {
            var funcNameMap = this.funcNameMap;
            if (funcNameMap.has(name)) {
                var nums = funcNameMap.get(name);
    ...
    Recorder.prototype.getFuncNameMap = function () {
