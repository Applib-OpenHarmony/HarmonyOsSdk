    ...
    HistogramStatistics.prototype.print = function () {
        ...
        this.insHistogram.forEach(function (value, key) {
            if (key.length < 8) { // 8 indicates insn name length
                log_1.LOGD(key + "\t\t\t" + value.getCount() + "\t\t" + value.getInstSize() + "\t\t" + value.getTotalSize() + "\t\t"
                    + value.getSavedSizeIfRemoved(_this) + "\t" + Math.round(value.getSavedSizeIfRemoved(_this) / totalSize * 100) + "%"); // multiplying 100 is to calculate percentage data
            }
            else if (key.length < 16) { // 16 indicates insn name length
                log_1.LOGD(key + "\t\t" + value.getCount() + "\t\t" + value.getInstSize() + "\t\t" + value.getTotalSize() + "\t\t"
                    + value.getSavedSizeIfRemoved(_this) + "\t" + Math.round(value.getSavedSizeIfRemoved(_this) / totalSize * 100) + "%");
            }
            else {
                log_1.LOGD(key + "\t" + value.getCount() + "\t\t" + value.getInstSize() + "\t\t" + value.getTotalSize() + "\t\t"
                    + value.getSavedSizeIfRemoved(_this) + "\t" + Math.round(value.getSavedSizeIfRemoved(_this) / totalSize * 100) + "%");
