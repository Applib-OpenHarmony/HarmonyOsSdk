...
var Compiler = /** @class */ (function () {
    ...
    Compiler.prototype.callOpt = function () {
        var _a, _b;
        var CallMap = new Map([
            ["this", 1],
            ["4newTarget", 2],
        ...
        var callType = 0;
        var scope = this.pandaGen.getScope();
        if (scope instanceof scope_1.FunctionScope) {
            var tempLocals = [];
            var tempNames_1 = new Set();
            var count = 0;
            // 4funcObj/newTarget/this
            for (var i = 0; i < 3; i++) {
                if (scope.getCallOpt().has(scope.getParameters()[i].getName())) {
                    tempLocals.push(this.pandaGen.getLocals()[i]);
                    callType += (_a = CallMap.get(scope.getParameters()[i].getName())) !== null && _a !== void 0 ? _a : 0;
                }
                else {
                    tempNames_1.add(scope.getParameters()[i].getName());
                    count++;
                }
            }
            // acutal parameters
            for (var i = 3; i < this.pandaGen.getLocals().length; i++) {
                tempLocals.push(this.pandaGen.getLocals()[i]);
            }
            var name2variable_1 = scope.getName2variable();
            name2variable_1.forEach(function (value, key) {
                if (tempNames_1.has(key)) {
                    name2variable_1["delete"](key);
                }
            });
            ...
            if (scope.getArgumentsOrRestargs()) {
                callType += (_b = CallMap.get("argumentsOrRestargs")) !== null && _b !== void 0 ? _b : 0;
            }
    ...
    Compiler.prototype.storeSpecialArg2LexEnv = function (arg) {
        ...
        if (v && v.isLexVar) {
            if ((arg === "this" || arg === "4newTarget") && variableInfo.scope instanceof scope_1.FunctionScope) {
                variableInfo.scope.setCallOpt(arg);
            }
            if (arg === "arguments" && variableInfo.scope instanceof scope_1.FunctionScope) {
                variableInfo.scope.setArgumentsOrRestargs();
            }
            var pandaGen = this.pandaGen;
            var vreg = "4funcObj" === arg ? vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.FUNC) : pandaGen.getVregForVariable(variableInfo.v);
    ...
    Compiler.prototype.setCallOpt = function (scope, callOptStr) {
        if (scope instanceof scope_1.FunctionScope) {
    ...
    Compiler.prototype.storeTarget = function (node, variable, isDeclaration) {
        if (variable.v instanceof variable_1.LocalVariable) {
            if (isDeclaration && variable.v.isLetOrConst()) {
                ...
                if (variable.scope instanceof scope_1.GlobalScope) {
                    if (variable.v.isLet()) {
            ...
            if (variable.v.isLetOrConst() && variable.scope instanceof scope_1.GlobalScope) {
